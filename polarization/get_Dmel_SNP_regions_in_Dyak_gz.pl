...
# takes a list of SNPs and uses blast to generate a bedfile coordinates in Dyak
...
system("rm check_blast_results_Dyak");

if (@ARGV < 2){print "\nusage: zcat reffile.z.Z | perl get_Dmel_SNP_regions_in_Dyak_gz.pl list buffer\n\n";exit;}
...
open OUT2, ">Dyak_snp_coordinates.bed";
for $j (0..scalar(@snps-1)){
    ...
    open TEMP2, "./blast/blastn -query temp.fa -db  Dyak_Tai18E2.fasta -outfmt 6 -max_target_seqs 1 2>/dev/null |";
    system("./blast/blastn -query temp.fa -db  Dyak_Tai18E2.fasta -outfmt 6 -max_target_seqs 1 2>/dev/null >>check_blast_results_Dyak");
    ...
    if (scalar(@lines)>0){ # if there is a blast hit
        ...
        $key = substr($key, 1);
        print "$snps[$j]\tDyak\t$arm\t$start\t$end\t$direction\t", $end-$start+1, "\n";
        print OUT2 "$snps[$j]\tDyak\t$arm\t$start\t$end\t$direction\t", $end-$start+1, "\n";
        $counter++;
        } # if there is a blast hit
    else{print "$snps[$j]\tDyak\tno_hit\n";; print OUT2 "$snps[$j]\tDyak\tno_hit\n";}
