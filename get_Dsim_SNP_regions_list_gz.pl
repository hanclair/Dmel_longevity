...
if (@ARGV < 1){print "\nusage: zcat reffile.z.Z | get_Dsim_SNP_regionslist_gz.pl list\n\n";exit;}
...
my @Dsim_seqs=();
open OUT, ">Dsim_snp_sequences.fa";
for $j (0..scalar(@snps-1)){
    ...
    else{push (@Dsim_seqs,"no_hit"); print OUT ">$snps[$j]\nno_hit\n";}
    } # for all snp regions
